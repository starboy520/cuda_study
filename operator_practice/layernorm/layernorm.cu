...
// ---------------------------------------------------------------
// 【公式 1：LayerNorm】—— 减均值 + 除标准差 + 仿射(gamma, beta)
//
//   μ    = (1/D) · Σ_i x[i]                    # 均值
//   σ²   = (1/D) · Σ_i (x[i] - μ)²             # 方差
//   x̂[i] = (x[i] - μ) / sqrt(σ² + eps)         # 标准化(零均值单位方差)
//   y[i] = x̂[i] · gamma[i] + beta[i]           # 仿射变换
//
//   特点：两遍归约(先求 μ，再求 σ²)，有 beta 平移项。
// ---------------------------------------------------------------
...
//
// ---------------------------------------------------------------
// 【公式 2：RMSNorm】—— 只除均方根(RMS)，不减均值、无 beta
//
//   ms   = (1/D) · Σ_i x[i]²                   # 均方(mean square)
//   rms  = sqrt(ms + eps)                       # 均方根
//   y[i] = x[i] / rms · gamma[i]                # 缩放(无平移)
//
//   与 LayerNorm 的区别：
//     · 不减均值 μ  → 只需一遍归约(直接求 Σx²)
//     · 没有 beta   → 少一个参数、少一次加法
//   所以更快，现代 LLM(LLaMA/DeepSeek)普遍用它替代 LayerNorm。
// ---------------------------------------------------------------
//   x      : [rows, D]  输入
//   gamma  : [D]        缩放参数（所有行共享）
//   y      : [rows, D]  输出
