...
注意：块内归约只在单个 block 内有效 → 必须用 <<<1, blockSize>>> 启动，
      且 blockSize 是 32 的倍数、≤1024。
             */
__global__ void softmaxRow(const float* input, int length, float* output) {
    ...
    float sum1 = 0.0;
    for (int i = tid; i < length; i+= stride) {
        sum1 += expf(input[i] - block_max);
    }
    sum1 = blockReduceSumF(sum1);
    if (threadIdx.x == 0) {
        total_sum = sum1;
    }
    __syncthreads();
    ...
    for (int i = tid; i < length; i+=stride) {
        output[i] = expf(input[i] - block_max) / total_sum;
    }
}
