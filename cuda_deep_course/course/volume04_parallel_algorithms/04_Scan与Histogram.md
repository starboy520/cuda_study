...
> 上极常用的操作（过滤、去重、稀疏数据打包都靠它），§3.5 会展开。
...
> "一个 block 负责的那一小段"就叫一个 tile。比如 100 万个数、每 block 管 1024 个，
> 就切成约 1000 个 tile。下面图里每个 block 下面的 `[3 1 2 4]` 就是一个 tile。
