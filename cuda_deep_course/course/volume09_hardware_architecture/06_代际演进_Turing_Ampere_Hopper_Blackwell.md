...
| **Blackwell** | B200/GB200, B300/GB300, RTX Blackwell | 10.0 / 10.3 / 12.x | 更强 Tensor Core、FP4/NVFP4、更大 NVLink（以官方为准）|
...
## 4. Blackwell（B200/GB200、B300/GB300、RTX Blackwell）

```text
关键演进（以官方公开为准）：
- 更强的 Tensor Core，支持更低精度（FP4/NVFP4 等），进一步提升 AI 吞吐
...
Compute Capability 要分产品线看：

```text
B200 / GB200 数据中心 GPU：CC 10.0
B300 / GB300 Blackwell Ultra：CC 10.3
RTX Blackwell / RTX PRO Blackwell：CC 12.x
```

所以不要只说“Blackwell 是 10.x”。面试时更好的说法是：

```text
Blackwell 是一代架构族，不同产品线 CC 不同；判断能否用某个 CUDA 特性，要查官方 CC 表和 tuning guide。
```
...
- Blackwell 数据中心 B200/GB200 是 CC 10.0，B300/GB300 是 CC 10.3；RTX Blackwell 是 12.x
...
FP4/NVFP4 等更低精度，越做越低精度换吞吐，专攻 AI；② 数据移动——传统手动搬 → Ampere cp.async 异步直达 →
