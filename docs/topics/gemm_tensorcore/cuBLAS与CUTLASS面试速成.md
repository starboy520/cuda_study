...
S 表示 single-precision real。它适合学习 GEMM 参数、做 FP32 基线和使用经典 BLAS。无需背几百个函数，命名规律见[附录：cuBLAS 函数速查](#附录cublas-函数速查)。
...
深入材料：[CUDA 深水区：PTX、SASS、MMA、异步流水与 Hopper](../performance/CUDA深水区_PTX_SASS_MMA_异步流水与Hopper.md)。
...
> 以下内容用于建立命名与选型心智模型；具体参数、类型支持、架构限制和版本行为等 API 语义以 [NVIDIA cuBLAS 官方文档](https://docs.nvidia.com/cuda/cublas/)为准。
>
