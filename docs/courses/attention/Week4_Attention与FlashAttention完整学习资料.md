...
先使用完整的矩阵下标。`S[i,j]` 表示第 `i` 个 query 与第 `j` 个 key 的匹配分数，`P[i,j]` 表示对应的注意力权重：
...
- `i`：当前 query 的编号；计算这一组 softmax 时，`i` 固定不变；
- `j`：当前正在计算权重的 key；`P[i,j]` 是这一行中的一个结果；
- `k`：在分母中遍历所有 key，把第 `i` 个 query 对所有 key 的指数分数加起来。

因此，公式表达的是：**固定第 `i` 个 query，也就是固定 score 矩阵的第 `i` 行，然后沿 key 维对这一整行做 softmax。** 它不是“对第 `j` 行做 softmax”；`j` 只是这一行中当前 key 的列下标。

后面的手算只研究一个固定的 query。为了减少下标，可以简写为：
