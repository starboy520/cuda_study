> 归档说明：本文迁移自 `gpu-kernel-engineering` 的 FP32 FlashAttention 基线阶段临时学习手册，保留其从 Attention 基础、Online Softmax 到 Tiled/`cp.async` 的完整学习路径。当前作品证据仍以 `gpu-kernel-engineering` 为准。
