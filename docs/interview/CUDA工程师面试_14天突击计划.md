...
> **状态：历史 / 备用验收资料（不是当前执行计划）**
>
> 本文按设计要求暂时保留，仅用于按需抽取验收题。当前执行计划的唯一入口是 [study_plan/README.md](../../study_plan/README.md)，当前四周计划是 [四周聚焦计划：AI Infra 与 CUDA 深水区](../../study_plan/四周聚焦计划_AIInfra与CUDA深水区.md)。
...
本文先给出共同基线和执行规则，再提供 Day 1～14 完整日程及附录；在历史 14 天突击场景下，可从“突击前准备”开始执行，并以每日验收而非阶段标题判断实际完成度。
...
3. 每题流程固定：计时 60～120 秒闭卷口答并写关键词；封存带时间戳的原始答案后，再查上述资料和追问答案；用不同颜色或 diff 写“原答、资料依据、修正版”。修正版只进入错题卡和学习记录，不能回填首轮答案或提高首轮得分。
4. 每题 10 分：结论 2、条件/边界 3、仓库实验/证据 3、追问应对 1、限时与表达 1。首轮每题 10 分和首轮总分只根据带时间戳的原始闭卷答案评分；评分者逐项勾选并写证据页码/录音时间戳。自己评分时必须保留原答和修订 diff，不能只填总分。硬错误（越界/死锁、把 profiler replay 当墙钟、把旧记录冒充本日重跑、用单指标作唯一因果结论）该题最高 4 分。
5. 首轮通过阈值为原始闭卷总分至少 112/140、七组均至少 14/20，且无硬错误；未达标的题进入错题卡，字段为“错误原句/反例或边界/可信证据/正确三段式/下次复测时间”。查证后至少间隔 60 分钟，随机抽 5 题，在不看资料和错题卡的情况下重新作答，才形成复测分；复测须达到 40/50 且无同类硬错误。首轮分与复测分并列保留，复测不得覆盖首轮分。
...
**优先级**：必须完成两道限时 kernel、三道 ncu 症状题、正确性/测试证据、评分和口述；A100 与 CUDA 编译/运行工具可用时，两道 kernel 都必须实际编译运行、通过 CPU/可信 reference 与边界测试，并运行适用的 sanitizer。性能优化和额外诊断实验是尽量项；时间不足可缩减参数扫描和第二轮优化，不能牺牲两题正确基础版的实际运行、三类诊断链或四类输出。
...
3. 每题必须说明 work/span 或主要运算复杂度，以及 algorithmic bytes 与预期 arithmetic intensity；测试至少含空/最小值、31/32/33 邻域、奇数大 shape、极值或近零数值场景，使用 CPU/可信 reference、finite、max_abs/max_rel 和适用的 sanitizer。A100 与 CUDA 工具可用时，两题都要保存编译命令、运行输出、边界对拍和 sanitizer 结果；未编译或未运行的题，其正确性、边界、测试项均为 0 分，且 Day 13 不得判通过。
...
7. 评分共 100 分：kernel 各 30 分（澄清 3、正确性/边界 12、同步 4、复杂度/bytes 3、测试 5、优化与表达 3）；诊断三题各 12 分（现象 1、候选原因 3、补充证据 2、单一可证伪实验 4、复测指标 2）；审计与限时 4 分。通过阈值 80/100、每道 kernel 至少 22/30、每道诊断至少 8/12，且无硬失败。只有明确记录 nvcc、GPU、驱动或权限阻塞时，才允许用伪代码做口头降级考察；它不算两道 kernel 完成、不满足 Day 13 通过条件，必须在次日或 3～7 天补救期补齐实际编译、运行、reference、边界与 sanitizer。

**必须产出**：封存抽题记录、两道独立 kernel 源码、两题的实际编译/运行命令与原始输出、接口澄清表、线程映射、边界与同步说明、复杂度/bytes、CPU/可信 reference、边界测试与适用 sanitizer、三张 ncu 诊断卡、一个实验前后或完整复测设计、逐项评分表、5 分钟录音。原始代码、命令和输出必须可供朋友复核；工具/硬件阻塞时另交阻塞记录与伪代码口试，但不替代上述必须产出。
...
6. 每轮 100 分：技术正确性 25、条件/边界 15、推理与取舍 15、代码/系统结构 15、实验与证据 15、表达/时间管理 10、追问与诚实边界 5。两位评分者可独立打分后记录分歧；只有自己时，通过录音时间戳逐项引用原话，不能凭印象填总分。项目 5 分钟版另做一个 **20 分投影**：问题/基线 4、正确性与测试 4、性能计时/指标证据 4、优化与失败实验 4、边界/库对比/下一步 4。该 20 分仅从第一轮已有的技术正确性、条件/边界、推理与取舍、实验与证据、表达等项目相关评分证据中映射复算，是第一轮项目表现的独立投影，不额外加进第一轮 100 分，第一轮总分仍为 100。
7. ready 阈值为两轮均至少 80/100、合计至少 165/200，且上述项目 20 分投影至少 16/20，无硬失败。评分表必须给项目五个 4 分项各自的录音时间戳/答题证据，并标出映射到第一轮 100 分 rubric 的来源，使第一轮总分和项目投影都可独立复算。硬失败为：kernel 明显越界/死锁且无法自纠；把错误结果用于性能结论；用单一 ncu 指标断言根因；伪造或混淆本人当天重跑与仓库旧记录；系统设计无正确性/容量/故障边界；无法说明项目中本人贡献。
